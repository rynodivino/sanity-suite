...
fastify.get('/badge/:workflow', async (request, reply) => {
  ...
  try {
    ...
    log(SERVICE_NAME, 'error', 'Failed to fetch workflow status', {
      error: error.message,
    })
...
fastify.get('/:release/:env.svg', async (request, reply) => {
  ...
  try {
    ...
    log(SERVICE_NAME, 'error', 'Failed to fetch badge status', {
      error: error.message,
    })
...
fastify.post('/:release/:env', async (request, reply) => {
  ...
  if (!status) {
    return reply
      .code(400)
      .send({ error: 'Status is required in the request body' })
  }
  ...
  try {
    ...
    log(SERVICE_NAME, 'info', `Status updated for ${release}/${env}`, {
      status,
    })
    reply.send({ message: `Status for ${release}/${env} updated to ${status}` })
  } catch (error) {
    log(SERVICE_NAME, 'error', 'Failed to update status', {
      error: error.message,
    })
...
const start = async () => {
  try {
    ...
    log(SERVICE_NAME, 'error', 'Failed to start service', {
      error: err.message,
    })
    process.exit(1)
  }
